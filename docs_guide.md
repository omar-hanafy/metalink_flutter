...
### B. Dartdoc Links vs. Literals (Avoid Broken References)
Square brackets in Dartdoc create links. If the target is not a real, public symbol,
you will get warnings and lose Pana points.
*   **Rule:** Use `[...]` only for real, resolvable API symbols.
*   **Members:** Use fully qualified links like `[ClassName.member]`.
*   **Literals:** Use backticks for values, numbers, and code fragments.
    *   **Good:** `Defaults to `Colors.grey.shade300`.`
    *   **Good:** `Accepts values between `0` and `1`.`
    *   **Bad:** `Defaults to [Colors.grey[300]].`
    *   **Bad:** `Accepts values between [0] and [1].`

### C. Define Constraints & Defaults
...
6.  [ ] Are all Dartdoc links resolvable (no warnings)?
